...
|ConditionalDLC|ModDesc Task Header or DLC Folder Name (String)|Conditional DLC to check for. Depending on your cmmver, you can use different value types here, [see below](#DLCRequirement-Structs) about `DLCRequirement` structs. This can be a ModDesc header (as listed above) or a DLC folder name like DLC_CON_BackOff. If using a header it will be resolved to the DLC folder name. The condition is checked against this DLC in the game's DLC folder and will cause this alt file to be applied or not applied when the mod is installed.|if any `Condition` except `COND_MANUAL` or `ModOperation` except `OP_NOTHING`|
