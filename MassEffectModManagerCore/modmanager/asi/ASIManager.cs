...
//        {
//            M3Log.Information(@"Loading ASI Manager manifest");
//            try
//            {
...
//            catch (Exception e)
//            {
//                M3Log.Error($@"Error loading ASI manifest: {e.Message}");
//            }
//        }
...
//            {
//                M3Log.Information(@"Fetching ASI manifest from online source");
...
//                catch (Exception e)
//                {
//                    M3Log.Error(@"Error writing cached ASI manifest to disk: " + e.Message);
//                }
...
//                catch (Exception e)
//                {
//                    M3Log.Error(@"Error parsing online manifest: " + e.Message);
//                    internalLoadManifest(true); //force local load instead
//                }
//            }
//            else if (File.Exists(ManifestLocation))
//            {
//                M3Log.Information(@"Loading ASI local manifest");
...
//                M3Log.Error(@"Cannot load ASI manifest: Could not fetch online manifest and no local manifest exists");
//            }
//        }
...
//        {
//            M3Log.Information($@"Using ASI manifest from disk: {manifestPath}");
...
//        /// <returns></returns>
//        public static bool InstallASIToTarget(ASIModVersion asi, GameTargetWPF target, bool? forceSource = null)
//        {
//            if (asi.Game != target.Game) throw new Exception($@"ASI {asi.Name} cannot be installed to game {target.Game}");
//            M3Log.Information($@"Processing ASI installation request: {asi.Name} v{asi.Version} -> {target.TargetPath}");
...
//            {
//                M3Log.Information(@"Creating ASI directory in game: " + destinationDirectory);
...
//                    {
//                        M3Log.Information($@"{v.AssociatedManifestItem.Name} is already installed. We will not remove the existing correct installed ASI for this install request");
...
//                    }
//                    M3Log.Information($@"Deleting existing ASI from same group: {v.InstalledPath}");
//                    v.Uninstall();
...
//                if (md5 == asi.Hash)
//                {
//                    M3Log.Information($@"Copying ASI from cached library to destination: {cachedPath} -> {finalPath}");

//                    File.Copy(cachedPath, finalPath, true);
//                    M3Log.Information($@"Installed ASI to {finalPath}");
...
//                M3Log.Information(@"Fetching remote ASI from server");
//                try
//                {
...
//                    {
//                        //ERROR!
//                        M3Log.Error(@"Downloaded ASI did not match the manifest! It has the wrong hash.");
//                        return false;
//                    }

//                    M3Log.Information(@"Fetched remote ASI from server. Installing ASI to " + finalPath);
//                    memoryStream.WriteToFile(finalPath);
//                    M3Log.Information(@"ASI successfully installed.");
//                    Analytics.TrackEvent(@"Installed ASI", new Dictionary<string, string>()
//                    {
//                        {@"Filename", Path.GetFileNameWithoutExtension(finalPath)}
//                    });
...
//                    {
//                        M3Log.Information(@"Creating cached ASIs folder");
//                        Directory.CreateDirectory(CachedASIsFolder);
//                    }

//                    M3Log.Information(@"Caching ASI to local ASI library: " + cachedPath);
//                    memoryStream.WriteToFile(cachedPath);
//                    return true;
//                }
//                catch (Exception e)
//                {
//                    M3Log.Error($@"Error downloading ASI from {asi.DownloadLink}: {e.Message}");
//                }
//            }
...
//        /// <param name="target"></param>
//        /// <returns></returns>
//        public static bool InstallASIToTarget(ASIMod mod, GameTargetWPF target)
//        {
...
//        public static bool InstallASIToTargetByGroupID(int updateGroup, string nameForLogging, GameTargetWPF gameTarget)
//        {
//            var group = GetASIModsByGame(gameTarget.Game).FirstOrDefault(x => x.UpdateGroupId == updateGroup);
//            if (group == null)
//            {
//                // Cannot find ASI!
//                M3Log.Error($@"Cannot find ASI with update group ID {updateGroup} for game {gameTarget.Game}");
//                return false;
//            }
